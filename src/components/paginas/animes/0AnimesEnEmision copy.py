...
def build_and_obfuscate_react_app():
    os.chdir("anime-web")
    subprocess.run(["npm", "run", "build"])
    main_js_file = "build/static/js/main.*.js"
    subprocess.run(["terser", main_js_file, "-o", main_js_file, "-c", "-m"])
    os.chdir("..")

def push_to_github():
    subprocess.run(["git", "init"])
    with open(".gitignore", "a") as gitignore:
        if "mi-app/build" not in gitignore.read():
            gitignore.write("mi-app/build\n")
    subprocess.run(["git", "add", "."])
    subprocess.run(["git", "commit", "-m", "Actualizar episodios"])
    subprocess.run(["git", "remote", "add", "origin", "https://github.com/rasta98409/AnimeWeb.git"])
